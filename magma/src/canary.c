...
void oracle_hit(int bug_id, int oracle_id, int step_id)
{
    (void)bug_id;
    (void)oracle_id;
    (void)step_id;
...
void magma_log(const char *bug, int condition)
{
    ...
    // 静态数组：跟踪当前执行中已输出的 bug
    static unsigned char logged[BUFFERLEN] = {0};
    size_t idx = prod_canary - data_ptr->producer_buffer;
